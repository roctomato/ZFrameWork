...
local app_logger = CS.UnityEngine.Debug

local __string = function (mod, cat, text)
    return "[" .. mod.name .. "][" .. cat .. "] " .. text
end
...
local function define_category(module, category)
    ...
    method_def = {
        ...
        info = function (...)
            if out_level and out_level.rawValue <= levels.INFO.rawValue and is_category_open(category) then 
                app_logger.Log(__string(module, category, concat_all(...)))
            end
        end, 
        ...
        warn = function (...)
            if out_level and out_level.rawValue <= levels.WARN.rawValue and is_category_open(category) then 
        ...
        fatal = function (...)
            if out_level and out_level.rawValue <= levels.FATAL.rawValue and is_category_open(category) then 
                app_logger.LogError(__string(module, category, concat_all(...)))
            end
        end, 
